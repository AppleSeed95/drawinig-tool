...
methods: {
    ...
    async resizeBackgroundImage()  {
        ...
        try {
            const blob = await fetch(this.backgroundImage).then((response) => response.blob());
    ...
    mousedown: function (event) {
        ...
        if (this.undoDataStack.length === 0 || !this.isImageDataEqual(this.undoDataStack[this.undoDataStack.length - 1], currentImageData)) {
    ...
    onTwoTouchStart: function (event) {
        ...
        this.initialStagePositions = this.initialPointerPositions.map(pointerPos => {
    ...
    mousemove: function (event) {
        ...
        this.localPos.x = (this.lastPointerPosition.x - this.drawingScope.x() - stagePos.x) / stageScale.x;
        this.localPos.y = (this.lastPointerPosition.y - this.drawingScope.y() - stagePos.y) / stageScale.y;
        ...
        this.localPos.x = (this.pos.x - this.drawingScope.x() - stagePos.x) / stageScale.x;
        this.localPos.y = (this.pos.y - this.drawingScope.y() - stagePos.y) / stageScale.y;
    ...
    onTwoTouchMove: function (event) {
        ...
        const oldMiddlePos = {
            x: (this.initialPointerPositions[0].x + this.initialPointerPositions[1].x) / 2,
            y: (this.initialPointerPositions[0].y + this.initialPointerPositions[1].y) / 2,
        };
        ...
        let newOffsetX = this.stage.x() + (middlePos.x - oldMiddlePos.x) + //位置
                        (this.stage.x() - middlePos.x) * scaleDiff / clampedScale; //スケール
        let newOffsetY = this.stage.y() + (middlePos.y - oldMiddlePos.y) +  //位置
                        (this.stage.y() - middlePos.y) * scaleDiff / clampedScale; //スケール

        // Constrain stage to its boundaries
        newOffsetX = Math.min(newOffsetX, 0);
        newOffsetX = Math.max(newOffsetX, -this.stage.width() * clampedScale + this.stage.width());

        newOffsetY = Math.min(newOffsetY, 0);
        newOffsetY = Math.max(newOffsetY, -this.stage.height() * clampedScale + this.stage.height());
